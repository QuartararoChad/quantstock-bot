# QuantStock Bot v4.0 - Clean rewrite
...
.card-bull{border-left:3px solid #00d464;} .card-bear{border-left:3px solid #ff3b5c;} .card-neut{border-left:3px solid #ffa500;}
...
.up{color:#00d464;font-size:0.85rem;} .dn{color:#ff3b5c;font-size:0.85rem;}
...
with st.sidebar:
    ...
    sidebar_html = "<div style='font-size:0.65rem;color:#4a6a8a;line-height:2;'>LAST UPDATE: " + last_dt + "<br>REFRESHES: " + str(st.session_state.refresh_count) + "<br>TICKERS: " + str(len(st.session_state.tickers)) + "</div>"
...
header_html = "<div class='hdr'><div><div class='app-title'>QuantStock</div><div class='app-sub'>Real-Time Signal Dashboard  |  " + now_str + "</div></div><div class='live-badge'><span class='live-dot'></span> LIVE</div></div>"
...
def add_indicators(df):
    ...
    for i in range(1, len(df)):
        ...
        pu = upper.iloc[i - 1]
        pc = c.iloc[i - 1]
        lower.iloc[i] = lr_raw.iloc[i] if (lr_raw.iloc[i] > pl or pc < pl) else pl
        upper.iloc[i] = ur_raw.iloc[i] if (ur_raw.iloc[i] < pu or pc > pu) else pu
        if c.iloc[i] > pu:
            stdir.iloc[i] = 1
        elif c.iloc[i] < pl:
            stdir.iloc[i] = -1
...
for ticker in st.session_state.tickers:
    ...
    st.markdown(build_card(ticker, direction, conf, sigs, rec, price, chg, chg_pct, rsi_val, macd_val, atr_val, vol_ratio), unsafe_allow_html=True)
    ...
    candle = go.Candlestick(x=df.index, open=df["Open"], high=df["High"], low=df["Low"], close=df["Close"], name="Price", increasing=dict(line=dict(color="#00d464", width=1), fillcolor="#00d46422"), decreasing=dict(line=dict(color="#ff3b5c", width=1), fillcolor="#ff3b5c22"))
    fig.add_trace(candle, row=1, col=1)

    sma20_trace = go.Scatter(x=df.index, y=df["SMA20"], name="SMA20", line=dict(color="#ff9800", width=1.5, dash="dot"))
    fig.add_trace(sma20_trace, row=1, col=1)

    if show_sma50:
        sma50_trace = go.Scatter(x=df.index, y=df["SMA50"], name="SMA50", line=dict(color="#ab47bc", width=1.5, dash="dot"))
        fig.add_trace(sma50_trace, row=1, col=1)

    if show_bb:
        bbu_trace = go.Scatter(x=df.index, y=df["BBU"], name="BB Upper", line=dict(color="rgba(100,180,255,0.4)", width=1), showlegend=False)
        fig.add_trace(bbu_trace, row=1, col=1)
        bbl_trace = go.Scatter(x=df.index, y=df["BBL"], name="BB Lower", fill="tonexty", fillcolor="rgba(100,180,255,0.05)", line=dict(color="rgba(100,180,255,0.4)", width=1), showlegend=False)
        fig.add_trace(bbl_trace, row=1, col=1)

    st_color = "#00d464" if df["STDIR"].iloc[-1] == 1 else "#ff3b5c"
    st_trace = go.Scatter(x=df.index, y=df["STLINE"], name="SuperTrend", line=dict(color=st_color, width=2))
    ...
    if show_vol:
        ...
        vc = ["#00d46455" if closes[i] >= opens[i] else "#ff3b5c55" for i in range(len(closes))]
        vol_trace = go.Bar(x=df.index, y=df["Volume"], name="Volume", marker_color=vc, showlegend=False)
        fig.add_trace(vol_trace, row=2, col=1)
        volsma_trace = go.Scatter(x=df.index, y=df["VOLSMA"], name="Vol SMA", line=dict(color="#ff9800", width=1), showlegend=False)
        fig.add_trace(volsma_trace, row=2, col=1)
        rsi_row = 3

    rsi_trace = go.Scatter(x=df.index, y=df["RSI"], name="RSI", line=dict(color="#7c4dff", width=1.5), fill="tozeroy", fillcolor="rgba(124,77,255,0.06)")
    fig.add_trace(rsi_trace, row=rsi_row, col=1)
    fig.add_hline(y=70, row=rsi_row, col=1, line=dict(color="#f
